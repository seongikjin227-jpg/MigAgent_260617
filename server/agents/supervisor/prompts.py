...
- Execute at most one job tool per Supervisor cycle.
- Job tools are run_data_migration, run_sql_conversion, run_sql_tuning, and run_sql_formatting.
- Do not call two job tools in the same assistant response.
- After one job tool finishes, do not call another job tool in the same cycle. End with flush_cycle_metrics() and request_wait().
"""
